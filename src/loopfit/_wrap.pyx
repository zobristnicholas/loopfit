...
cdef calibrate_vectorized(np.ndarray[float_t, ndim=1] f, np.ndarray[float64_t, ndim=1] i,
    ...
    for ii in range(f.shape[0]):
        calibrate_c(f[ii], i[ii], q[ii], center, fm, pr, pb, pi, po)
...
def calibrate(f, i=None, q=None, *,
    ...
    Args:
        ...
        center: boolean (optional)
            If True, the loop will also be centered on the origin making it
            easy to calculate the phase angle. (z -> center - z)
        fm:  float (optional)
...
def guess(f, i=None, q=None, *,
    ...
    Args:
        ...
        optional keyword arguments:
            ...
            used in the guess. 'fm' and 'decreasing' may be specified to
            fix those values for the fit.

    Returns:
        params: dictionary
            A dictionary of parameter values to use in the fit. 'fm' and
            'decreasing' are also included so that baseline parameters and the
            sweep direction are properly set for the fit.
    ...
    Returns:
        result: dictionary
            ...
            max_iterations are included in the dictionary along with
                ...
                guess: dictionary
                    A dictionary with the initial starting parameters and
                    everything else required to evaluate the model.
                summary: string
                    A fit report generated by ceres-solver.
                size: integer
                    The number of independent data points used in the fit
                    (2 * f.size).
                varied: integer
